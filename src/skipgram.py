...
class NegativeSamplingLoss(nn.Module):
    ...
    def forward(self, input_vectors: torch.Tensor, output_vectors: torch.Tensor,
        ...
        # Reshape input vectors to (batch_size, embed_size, 1) for batch matrix multiplication
        input_vectors = input_vectors.unsqueeze(2)  # (batch_size, embed_size, 1)

        # Compute log-sigmoid loss for correct classifications (positive samples)
        pos_dot = torch.bmm(output_vectors.unsqueeze(1), input_vectors).squeeze(2)  # (batch_size, 1)
        out_loss = torch.log(torch.sigmoid(pos_dot))  # (batch_size, 1)

        # Compute log-sigmoid loss for incorrect classifications (negative samples)
        neg_dot = torch.bmm(noise_vectors, input_vectors).squeeze(2)  # (batch_size, n_samples)
        noise_loss = torch.log(torch.sigmoid(-neg_dot)).sum(dim=1, keepdim=True)  # (batch_size, 1)
